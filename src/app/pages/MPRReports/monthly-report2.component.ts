...
@Component({
  selector: 'app-monthly-report2',
  templateUrl: './monthly-report2.component.html',
  styleUrls: ['./monthly-report2.component.scss']
})
export class MonthlyReport2Component implements OnInit {
  ...
  GetCMMMonthlyPerformancereport1(report: ReportInputModel) {
    ...
    this.spinner.show();
    this.paService.GetCMMMonthlyPerformance2(report).subscribe(data => {
        this.spinner.hide();
...
ExportTOExcel() {
  ...
  XLSX.writeFile(wb, 'MonthlyPerformanceReport2.xlsx');
} 
showHideFilterBlock() {
